...
class AIAnalyzer:
    ...
    def _load_dataset_insights(self) -> Dict:
        ...
        try:
            ...
            insights = data_processor.generate_development_insights()
            return {
                "data_analysis": insights,
                "child_utterances": data["child_utterances"][:100],  # 샘플만
                "conversation_pairs": data["conversation_pairs"][:50]  # 샘플만
            }
        except Exception as e:
            print(f"데이터셋 로딩 실패: {e}")
            return {"data_analysis": {}, "child_utterances": [], "conversation_pairs": []}
...
"""
    ...
    def analyze_text_message(self, text: str, child_age_months: int, 
                           user_id: int, development_stage: str) -> Dict:
        """텍스트 메시지 AI 분석"""
        try:
            print(f"텍스트 분석 시작: '{text}' (발달 단계: {development_stage})")
            
            # 발달 단계별 분석 프롬프트 생성
            prompt = self.create_text_analysis_prompt(text, child_age_months, development_stage)
            ...
            # JSON 파싱
            analysis_result = json.loads(response.choices[0].message.content)
            print(f"OpenAI 분석 완료: {analysis_result}")
    ...
    def create_text_analysis_prompt(self, text: str, child_age_months: int, 
                                  development_stage: str) -> str:
        """텍스트 분석을 위한 프롬프트 생성 (데이터셋 기반)"""
        
        # 데이터셋 기반 컨텍스트 추가
        dataset_context = ""
        if self.dataset_insights.get("data_analysis"):
            insights = self.dataset_insights["data_analysis"]
            dataset_context = f"""
실제 아동 언어 발달 데이터셋 분석 결과:
- 발달 단계: {insights.get('language_development_stage', '분석 중')}
- 주요 관찰사항: {', '.join(insights.get('key_observations', []))}
- 권장사항: {', '.join(insights.get('recommendations', []))}
"""
        
        # 유사한 발화 예시 추가
        similar_utterances = ""
        if self.dataset_insights.get("child_utterances"):
            similar = [u for u in self.dataset_insights["child_utterances"] 
                      if len(u) <= len(text) + 2 and len(u) >= len(text) - 2][:5]
            if similar:
                similar_utterances = f"""
유사한 발화 예시: {', '.join(similar)}
"""
